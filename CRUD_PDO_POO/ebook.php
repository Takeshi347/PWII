<?php
    ...
    class Ebook {
        ...
        public function atualizar($id, $titulo, $descricao, $isbn, $data_publicacao, $preco, $idioma, $foto) { 
            $stmt = $pdo->prepare("UPDATE usuario SET titulo = :titulo, descricao = :descricao, isbn = :isbn, data_publicacao = :data_publicacao, preco = :preco, idioma = :idioma, foto = :foto, tipo = :tipo WHERE id = :id");
            $stmt->bindParam(':titulo', $titulo);
            $stmt->bindParam(':descricao', $descricao);
            $stmt->bindParam(':isbn', $isbn);
            $stmt->bindParam(':data_publicacao', $data_publicacao);
